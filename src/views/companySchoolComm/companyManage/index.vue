  ...
  <div class="patent_store_box demand_box">
    ...
    <div class="tab">
      ...
      >
        ...
        <el-table-column prop="nickName" label="企业名称" width="180">
          <template slot-scope="{ row }">
            <img v-if="row.img" class="avtar" :src="row.img" />
        ...
        <el-table-column prop="createDate" label="公司地址">
          <template slot-scope="{ row }">
...
import { getPriseschoolList } from "@/api/companyManage";
...
export default {
  ...
  methods: {
    ...
    goAdd(id) {
      this.$router.push({ path: "/addCompanyInfo", query: { id } });
    },
    getQuery(query) {
      this.query = query;
      this.getTableData();
